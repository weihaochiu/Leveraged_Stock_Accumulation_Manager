        ...
        for r, row in enumerate(rows):
            for c, (key, _, _) in enumerate(COLUMNS):
                ...
                if key == "strategy_status":
                    colors = {StrategyStatus.TARGET_REACHED: "#FECACA", StrategyStatus.NEAR_TARGET: "#FEF3C7", StrategyStatus.FREE_SHARES: "#D1FAE5", StrategyStatus.COMPLETED_WITH_TOLERANCE: "#DBEAFE"}
                    if value in colors: item.setBackground(QColor(colors[value]))
                if key == "current_return_pct": item.setForeground(QColor("#DC2626" if float(value or 0) >= 0 else "#059669"))
...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self):
        ...
        self._button(toolbar,"新增買入",lambda:self.open_dialog(BuyDialog(self.repository,self)))
    ...
    @staticmethod
    def _button(toolbar, text, callback):
