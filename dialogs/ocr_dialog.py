...
class OCRWorkbenchDialog(QDialog):
    def __init__(self, repository: PortfolioRepository, parent=None):
        ...
        note.setWordWrap(True); note.setProperty("role","secondary"); root.addWidget(note)
        actions=QHBoxLayout(); confirm=QPushButton("以正式表單確認選取列"); confirm.setProperty("role","primary"); confirm.clicked.connect(self.confirm_current); close=QPushButton("關閉"); close.clicked.connect(self.accept); actions.addWidget(confirm); actions.addStretch(); actions.addWidget(close); root.addLayout(actions)
